...
tuic_cert(){
    ...
    if [[ $certInput == 2 ]]; then
        ...
        mkdir /etc/hysteria >/dev/null 2>&1
...
inst_tuv5(){
    ...
    wget --no-check-certificate https://github.com/EAimTY/tuic/releases/download/tuic-server-1.0.0/tuic-server-1.0.0-x86_64-unknown-linux-gnu -O /usr/local/bin/tuic
...
EOF
    ...
    url="tuic://$uuid:$passwd@$domain:$port?congestion_control=bbr&udp_relay_mode=quic&alpn=h3&sni=$hy_domain&allow_insecure=1#tuicv5-misaka"
